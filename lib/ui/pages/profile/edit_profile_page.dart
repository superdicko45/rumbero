...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  _showSnackBar(String _text, bool _error) async {
    var _snackBar = SnackBar(content: Text(_text));
            ...
            return TextFormField(
              ...
              decoration: new InputDecoration(
                ...
                border: new OutlineInputBorder(
                  ...
                  borderSide:
                      new BorderSide(color: Theme.Colors.loginGradientStart),
                ),
              ),
    ...
    return Padding(
      ...
      child: InputDecorator(
        decoration: InputDecoration(
          ...
          labelText: '¿Con cual te identificas más?',
        ),
  ...
  Widget _iCiudad(List<General> _items) {
    return Padding(
      padding: EdgeInsets.all(16),
      child: InputDecorator(
        decoration: InputDecoration(
          icon: Icon(Icons.my_location),
          labelText: 'Cambiar locación',
        ),
        child: StreamBuilder<int>(
            stream: _editBloc.cityStream,
            builder: (context, snapshot) {
              return DropdownButtonHideUnderline(
                  child: DropdownButton<int>(
                value: snapshot.hasData ? snapshot.data : 1,
                isDense: true,
                onChanged: (int _newValue) {
                  _editBloc.changeCity(_newValue);
                },
                items: _items.map((General _tipo) {
                  return DropdownMenuItem<int>(
                    value: _tipo.itemId,
  ...
  Widget _alert(List<General> _items) {
    return AlertDialog(
      scrollable: true,
      title: Text('Mis actuales géneros'),
      actions: <Widget>[
        FlatButton(
            onPressed: () => Navigator.of(context).pop(), child: Text('Cerrar'))
      ],
      content: Container(
        child: SingleChildScrollView(
          child: StreamBuilder<List<int>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData)
                  return Wrap(children: _choiceChips(_items, snapshot.data));
                else
                  return Center(
                    child: Text('Sin géneros'),
                  );
              }),
        ),
      ),
    ...
    _items.forEach((element) {
      ...
      _chips.add(Container(
        ...
        child: ChoiceChip(
            ...
            labelStyle: _selected ? TextStyle(color: Colors.white) : null,
            selectedColor: Theme.Colors.loginGradientStart,
            selected: _selected,
            onSelected: (bool value) =>
                _editBloc.changeGenero(element.itemId, value)),
      ));
    });
          ...
          return MaterialButton(
              child: Padding(
                ...
                child: Text(
                  "Actualizar",
                  style: TextStyle(
                      color: snapshot.hasData ? Colors.white : Colors.grey,
                      fontSize: 25.0,
                      fontFamily: "WorkSansBold"),
                ),
              ),
              onPressed: snapshot.hasData ? () => _saveData() : null);
        });
  }
}
