...
class FilterBloc{
  ...
  final _gDispController = BehaviorSubject<List<General>>(); // Generos disponibles
  final _cDispController = BehaviorSubject<List<General>>(); // ciudades disponibles
  ...
  Future loadData(Search _filtro) async {
    ...
    if(_filtro != null){
      ...
      _tipoController.sink.add(_filtro.tipo ?? 1);
      _ciudadController.sink.add(_filtro.ciudad ?? 1);
      _minDController.sink.add(_filtro.minD);
      _maxDController.sink.add(_filtro.maxD);
      _tipoEController.sink.add(_filtro.tipoE ?? 0);
      _onlineController.sink.add(_filtro.online ?? false);
      _weekController.sink.add(_filtro.week ?? false);
      _weekendController.sink.add(_filtro.weekend ?? false);

      if(_filtro.min != null && _filtro.max != null ) 
        _rangeController.sink.add([_filtro.min,_filtro.max,]);
